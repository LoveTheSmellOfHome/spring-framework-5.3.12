 ...
 * @see Errors
 */
// 实用程序类提供调用Validator和拒绝空字段的便捷方法。
// 当使用rejectIfEmpty或rejectIfEmptyOrWhitespace时，在Validator实现中检查空字段可以成为rejectIfEmptyOrWhitespace 。
public abstract class ValidationUtils {
	 ...
	 */
	// 为提供的对象和Errors实例调用给定的Validator 。委派操作
	 ...
	 * {@link Validator#supports(Class) support} the validation of the supplied object's type
	 */
	// 为提供的对象和 Errors 实例调用给定的 ValidatorSmartValidator。
	 ...
	 */
	// 如果值为空，则拒绝具有给定错误代码的给定字段。此上下文中的“空”值表示 null 或空字符串“”。
	// 字段被验证的对象不需要传入，因为 Errors 实例可以自己解析字段值（它通常会保存对目标对象的内部引用）。
